...
void GameManager::Chapter1(std::vector<GameScene>& script) {
	...
	if (choice == 1) {
		...
		addScene(branch, "여자친구", "", "와... 저 사진들 진짜 징그럽다..", "여자친구가 게임 속 사진들을 바라봅니다.");
		addScene(branch, "여자친구", "", "근데 저 사진안에 있는 귀걸이..", "여자친구가 게임 속 귀걸이를 유심히 바라봅니다.");
		addScene(branch, "여자친구", "", "내가 어제 잃어버린 거랑 되게 비슷하지 않아?", "여자친구가 게임 속 귀걸이를 유심히 바라봅니다.");
	}
	else if(choice == 2) {
